...
const Index: React.FC<{
	...
	const [coeiroinkPath, setCoeiroinkPath] = useState("");
	const [isCoeiroinkDir, setIsCoeiroinkDir] = useState(false);
	const [checkedIsCoeiroinkDir, setCheckedIsCoeiroinkDir] = useState(false);
	...
	useDebounce(
		() => {
			(async () => {
				const isCoeiroinkDir = await invoke<boolean>("is_coeiroink_dir", {
					path: coeiroinkPath,
				});
				return isCoeiroinkDir;
			})().then((result) => {
				setCheckedIsCoeiroinkDir(true);
				setIsCoeiroinkDir(result);
			});
		},
		1000,
		[coeiroinkPath],
		...
		<div className="flex flex-col gap-4">
			...
			<section className="flex flex-col gap-2">
				...
				<input
					...
					value={coeiroinkPath}
					onChange={(e) => {
						setCheckedIsCoeiroinkDir(false);
						setCoeiroinkPath(e.target.value);
					}}
					className="input"
				/>
				...
				<p
					className={clsx(
						"text-xs",
						checkedIsCoeiroinkDir && !isCoeiroinkDir ? "text-accent" : "",
					)}
				>
					{checkedIsCoeiroinkDir
						? isCoeiroinkDir
							? "このフォルダはCoeiroinkのフォルダです。"
							: "このフォルダはCoeiroinkのフォルダではありません。"
						: "フォルダを確認中..."}
				</p>
				...
				<Toast.Provider>
					<button
						type="button"
						className="button"
						disabled={!(checkedIsCoeiroinkDir && isCoeiroinkDir)}
						onClick={async () => {
